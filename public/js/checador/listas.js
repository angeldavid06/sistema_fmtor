...
const render_lista_semanal = (horario) => {
    ...
    for (let i = 0; i < empleados.length; i++) {
        ...
        const semana = ['','','','','','']
        if (horario.length > 0) {
            if (horario[0][0][0] == empleados[i]) {
                console.log(i,true);
            }
            // for (let j = 0; j < semana.length; j++) {
            //     console.log(horario[0][j][0]);
                // if (horario[0][0][j].split(' ').length >= 2 && empleados[i][0] == horario[0][0][0]) {
                //     console.log(true,empleados[i][0],horario[0][0][j]);
                //     const fecha = horario[0][0][1][j].split(' ')[0]
                //     const dia_semana = new Date(fecha.split('-')[0],fecha.split('-')[1]-1,fecha.split('-')[2])
                //     if (dia_semana.getDay() == 2) {
                //         semana[0] = horario[0][0][1][j].split(' ')[1]
                //     } else if (dia_semana.getDay() == 3) {
                //         semana[1] = horario[0][0][1][j].split(' ')[1]
                //     } else if (dia_semana.getDay() == 4) {
                //         semana[2] = horario[0][0][1][j].split(' ')[1]
                //     } else if (dia_semana.getDay() == 5) {
                //         semana[3] = horario[0][0][1][j].split(' ')[1]
                //     } else if (dia_semana.getDay() == 6) {
                //         semana[4] = horario[0][0][1][j].split(' ')[1]
                //     } else if (dia_semana.getDay() == 1) {
                //         semana[5] = horario[0][0][1][j].split(' ')[1]
                //     }
                // }
            // }
        }
...
const obtener_registro = () => {
    let aux = []
    let horario = []
    let horarios = []
    const respuesta = fetchAPI('',url+'/checador/horarioController/obtener_registro?fecha_in='+rango_minimo.value+'&fecha_fin='+rango_maximo.value,'')
    respuesta.then(json => {
        if (json.length > 0) {
...
const generar_lista_semanal = () => {
    ...
    respuesta.then(json => {
        ...
        obtener_registro()
        // for (let e = 0; e < id_empleados.length; e++) {
        // }
    })
}
