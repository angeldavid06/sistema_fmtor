const render_clientes = (json) => {
    ...
    json.forEach(element => {
        if (element.Id_Clientes != 1) {
            body[0].innerHTML += '<tr>'+
                                    ...
                                    '<td><button class= "material-icons btn btn-icon-self btn-rojo" onclick="eliminarRegistro('+element.Id_Clientes+')">delete</button>'
                                  +'</tr>';
        }
      });
  }
  ...
  const eliminarRegistro =(id)=>{
      const respuesta = fetchAPI('',url+'/ventas/clientes/eliminarCliente?dato='+id,'')
      respuesta.then(json => {
          obtener();
      })
